<div class="wrapper wrapper-content animated fadeInRight">
    <div class="ibox float-e-margins">
        <div class="ibox-content">
            <form id="formSearch" class="form-inline form-box">
                <div class="form-group">
                    <label for="search_code">终端编号：</label>
                    <input type="text" class="form-control" id="search_code" placeholder="请输入终端名称" autocomplete="off" />
                </div>
                <div class="form-group">
                    <label for="search_code">终端名称：</label>
                    <input type="text" class="form-control" id="search_code" placeholder="请输入终端名称" autocomplete="off" />
                </div>
                <button type="button" class="btn btn-primary search-btn" id="btn_query">查询</button>
            </form>
            <div id="toolbar" class="btn-group ">
                <button type="button" id="btn_add" class="btn btn-primary">新增终端</button>
        ...
        <form id="uploadForm" action="<?php echo $ajax_url;?>/terminal_output" target="uploadFile" method="post" enctype="multipart/form-data">
            <input name="terminal_file" id="terminal_file" type="file" style="display: none;"/>
        </form>
...
<script>
    ...
    var TableInit = function() {
        ...
        oTableInit.Init = function() {
            $('#table').bootstrapTable({
                url: '<?php echo $ajax_url; ?>/get_terminal_list',// 请求后台的URL（ * ）
                method: 'post', //请求方式（*）
        ...
        oInit.Init = function() {
            ...
            $('#btn_add').on('click', function() {
                ...
                parent_frame.showLayer({
                    title: '新增终端信息',
                    content: '<?php echo $page_url;?>/terminal_list/add'
                }, '800px', '460px');
            });
            $('#btn_template').on('click', function(){
                location.href = '<?php echo STATIC_URL."/directory/terminal.xlsx";?>';
            });
            $('#btn_input').on('click', function(){
                console.log('upload_file');
                //parent_frame.layer.alert('正在建设中');
                $('#terminal_file').on('change', function(){
    ...
    function update(id){
        ...
        parent_frame.showLayer({
            title: '编辑终端信息',
            content: '<?php echo $page_url;?>/terminal_list/update/'+id
        }, '800px', '460px');
    }
    ...
    function remove(id)
    {
        ...
        parent_frame.layer.confirm('您确定要删除这个终端信息？',
        {
            btn: ['确认','取消'], //按钮
        ...
        function()
        {
            ...
            $.ajax({
                type: 'post',
                data: {'id': id},
                url: '<?php echo $ajax_url;?>/remove_terminal/',
                dataType: 'json',
